...
You can see the Kubernetes contexts available, we'll be using `docker-for-desktop` context.
![Kubernetes contexts](./kubernetes-desktop/images/k8s_contexts.png)
...
First check if Docker is running with Kubernetes or Docker Swarm as the orchestrator - Docker for Mac supports both orchestrators **at the same time**:

![Kubernetes Preferences](./kubernetes-desktop/images/k8s_prefs.png).

Ensure it is set to Kubernetes by selecting the correct radio button above.

> You can also switch orchestrators with the `DOCKER_ORCHESTRATOR` environment variable, setting it to `kubernetes` or `swarm`.
...
Now let us deploy the app to Kubernetes as a stack using the [compose file](./kubernetes-desktop/docker-compose-k8s.yml):

> If you were using the swarm orchestrator, the command to deploy the stack is exactly the same.

```
docker stack deploy -c docker-compose-k8s.yml voting-app
```

This pulls all the Docker images for the sample app, so it will take a few moments. The Docker command line logs progress so you can see the services and pods being created. When the stack is running you will see the message `Stack voting-app is stable and running`.

Docker for Mac and Docker for Windows include the [kubectl](https://kubernetes.io/docs/reference/kubectl/overview/) command line client, so you can work directly with the Kube cluster. Check the services are up, and you should see output like this:
...
Check that the pods are running, and you should see one pod each for each component in the application:
...
Browse to [http://localhost:5000](http://localhost:5000) to see the voting page and [http://localhost:5001](http://localhost:5001) to see the results, served by Docker containers running in Kubernetes.
...
You can deploy the same app to Kubernetes using this [Kubernetes manifest](./kubernetes-desktop/kube-deployment.yml). That describes the same application in terms of Kubernetes deployments, services and pod specifications.
...
Note that the database service is a headless service set by `clusterIP: None` in the spec, where the DNS is configured to return multiple address for the Service name that points directly to the Pods that back the service.
...
Now again browse to [http://localhost:5000](http://localhost:5000) and [http://localhost:5001](http://localhost:5001) you will see the same application.
